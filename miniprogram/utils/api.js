...
const miniprogramLogin = (code) => {
  return request.post('/auth/miniprogram', { code });
};
 ...
 * 提交实名认证
 */
const submitRealNameVerify = (data) => {
  return request.post('/verification/initiate', data);
};
...
const getVerifyResult = () => {
  return request.get('/verification/result');
};
...
const getVerifyHistory = () => {
  return request.get('/verification/history');
};
 ...
 * 创建小程序支付订单（新）
 */
const createMinipPayment = (data) => {
  return request.post('/payment/wechat/minip/create', data);
};
 ...
 * 获取钱包统计数据
 */
const getWalletStatistics = (period) => {
  return request.get('/wallet/statistics', { period });
};

/**
 * 获取月度账单
 */
const getMonthlyBill = (year, month) => {
  return request.get('/wallet/bills', { year, month });
};
...
module.exports = {
  // 认证
  miniprogramLogin,
  wechatLogin: miniprogramLogin, // 别名，保持兼容性
  bindPhone,
  manualBindPhone,
  sendSmsCode,
  getUserInfo,
