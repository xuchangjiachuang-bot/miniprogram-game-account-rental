...
const reportPath = path.join(rootDir, 'tmp', 'wechat-jsapi-click-report.json');
...
try {
  ...
  await page.addInitScript(() => {
    ...
    window.__wxMockEvents = [];
    window.wx = {
      config(config) {
        window.__wxMockEvents.push({ type: 'config', config });
        setTimeout(() => {
          listeners.ready.forEach((fn) => fn());
        }, 10);
      },
      ready(fn) {
        listeners.ready.push(fn);
      },
      error(fn) {
        listeners.error.push(fn);
      },
      checkJsApi({ success }) {
        const result = { checkResult: { chooseWXPay: true } };
        window.__wxMockEvents.push({ type: 'checkJsApi', result });
        setTimeout(() => success(result), 10);
      },
      chooseWXPay(payload) {
        window.__wxMockEvents.push({ type: 'chooseWXPay', payload });
        setTimeout(() => {
          payload?.success?.({ errMsg: 'chooseWXPay:ok' });
          payload?.complete?.({ errMsg: 'chooseWXPay:ok' });
        }, 10);
      },
    };
  });
  ...
  await page.waitForFunction(() => {
    ...
    return bodyText.includes('立即微信支付');
  }, { timeout: 15000 });

  await page.click('button:has-text("立即微信支付")');
  await page.waitForTimeout(5000);

  report.wxEvents = await page.evaluate(() => window.__wxMockEvents || []);
  report.final = {
    url: page.url(),
