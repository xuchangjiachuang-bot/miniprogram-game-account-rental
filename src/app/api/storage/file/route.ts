...
export async function GET(request: NextRequest) {
  ...
  const fileReference = classifyStoredFileReference(fileKey);
  const fileBuffer = await readFile(fileKey);
  if (!fileBuffer) {
    return NextResponse.json({
      success: false,
      error: fileReference.kind === 'storage-key'
        ? '文件不存在，可能是历史本地文件已丢失'
        : '文件不存在',
    }, { status: 404 });
  }
  ...
  return new NextResponse(new Uint8Array(fileBuffer), {
    status: 200,
    headers: {
      'Content-Type': inferContentType(fileReference.normalized || fileKey),
