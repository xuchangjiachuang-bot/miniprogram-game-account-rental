...
export async function cancelOrder(orderId: string, reason: string = '用户取消'): Promise<CancelOrderResult> {
  ...
  try {
    ...
    sendNotification({
      ...
      title: '订单已取消',
      content: isUnpaidCancellation
        ? `您的订单"${order.order_no}"已取消，订单未支付，无需退款`
        : `您的订单"${order.order_no}"已取消，退款¥${order.total_amount.toFixed(2)}已到账`,
      extras: {
        orderInfo: {
          orderNo: order.order_no,
        }
    ...
    return {
      success: true,
      message: isUnpaidCancellation ? '订单已取消' : '订单已取消，退款成功',
